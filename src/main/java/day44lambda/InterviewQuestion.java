...
public class InterviewQuestion {
    public static void main(String[] args) {
        //Example 1: Size verilen Array'in "Mountain Array" olup olmadığını gösteren kodu yazınız.
        //örnek [0,2,5,3,1,-2]

        int []arr={0,2,5,3,1,-2};
        //Array'i List'e çevirelim çünkü list' kullanmak daha kolay
        List<Integer> myList = new ArrayList<>();
        Arrays.stream(arr).forEach(t-> myList.add(t));
        System.out.println(myList);

        //List'te ki max elemanı bulalım.
        int max=myList.stream().reduce(Math::max).get();
        System.out.println(max);

        //List'te ki max elemanın index'ini bulalım
        int idxOfMax=myList.indexOf(max);
        System.out.println(idxOfMax);

        //Max elemana kadar ki tum elemanları bir list'in içine koy
        ...
        //Max elemandan sonraki tum elemanları bir list'in içine koy
        List<Integer> secondList=new ArrayList<>();
        myList.stream().filter(t->myList.indexOf(t)>=idxOfMax).forEach(t->secondList.add(t));
        System.out.println(secondList);

        //secondList'in copy'sini oluştur.
        ...
        //secondListCopy'i ters olarak sırala
        Collections.sort(secondListCopy);
        Collections.reverse(secondListCopy);
        System.out.println(secondListCopy);

        //secondList ile secondListCopy aynı mı diye kontrol et
        boolean rule2=secondList.equals(secondListCopy);
        System.out.println(rule2);

        System.out.println("İs the array mountain array? "+(rule1&& rule2));
        /*
